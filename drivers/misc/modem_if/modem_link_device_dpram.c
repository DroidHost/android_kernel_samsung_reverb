...
{
	const u16 init_start = INT_CMD(INT_CMD_INIT_START);
	const u16 init_end = INT_CMD(INT_CMD_INIT_END);
	u16 magic;
	u16 enable;

	/* P120622-0205, DPRAM sync issue when ril restart */
	dpram_write_command(dpld, init_start);
	pr_info("[LNK] <%s> Send INIT_START CMD\n", __func__);
	...
	pr_info("[LNK] <%s> Send INIT_END CMD\n", __func__);
