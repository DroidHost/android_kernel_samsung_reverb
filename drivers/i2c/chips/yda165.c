 ...
 *
 ******************************************************************************/
static void D4Hp3_UpdateRegisterMapN(SINT32 sdRetVal, UINT8 bRN,
...
void D4Hp3_WriteRegisterBit(UINT32 dName, UINT8 bPrm)
{
	...
	*/
	...
	/* check arguments */
	if (bRA < D4HP3_MIN_REGISTERADDRESS) {
		if (D4HP3_MAX_WRITE_REGISTERADDRESS < bRA)
			d4ErrHandler(D4HP3_ERROR_ARGUMENT);
	}
	/* set register parameter */
	bPrm = (bPrm << bSB) & bMB;
	bDummy = bMB ^ 0xFF;
	bWritePrm = g_bD4Hp3RegisterMap[bRN] & bDummy;
...
void D4Hp3_WriteRegisterByte(UINT8 bAddress, UINT8 bPrm)
{
	UINT8 bNumber;
	/* check arguments */
	if (bAddress < D4HP3_MIN_REGISTERADDRESS) {
		if (D4HP3_MAX_WRITE_REGISTERADDRESS < bAddress)
			d4ErrHandler(D4HP3_ERROR_ARGUMENT);
	}
...
void D4Hp3_WriteRegisterByteN(UINT8 bAddress, UINT8 *pbPrm, UINT8 bPrmSize)
{
	...
	UINT8 bNum_Reg;
	/* check arguments */
	bNum_Reg = D4HP3_MAX_WRITE_REGISTERADDRESS - D4HP3_MIN_REGISTERADDRESS;
	if (bAddress < D4HP3_MIN_REGISTERADDRESS) {
		if (D4HP3_MAX_WRITE_REGISTERADDRESS < bAddress)
			d4ErrHandler(D4HP3_ERROR_ARGUMENT);
	}
	...
	bNumber = bAddress - 0x80;
	D4Hp3_UpdateRegisterMapN(d4WriteN(bAddress, pbPrm, bPrmSize),
...
void D4Hp3_ReadRegisterBit(UINT32 dName, UINT8 *pbPrm)
{
	...
	*/
	...
	/* check arguments */
	if (bRA < D4HP3_MIN_REGISTERADDRESS) {
		if (D4HP3_MAX_READ_REGISTERADDRESS < bRA)
			d4ErrHandler(D4HP3_ERROR_ARGUMENT);
	}
...
void D4Hp3_ReadRegisterByte(UINT8 bAddress, UINT8 *pbPrm)
{
	...
	/* check arguments */
	if (bAddress < D4HP3_MIN_REGISTERADDRESS) {
		if (D4HP3_MAX_READ_REGISTERADDRESS < bAddress)
			d4ErrHandler(D4HP3_ERROR_ARGUMENT);
	}
	bNumber = bAddress - 0x80;
 ...
 *(0 : no check,  1 : check)
 *			D4HP3_SETTING_INFO *pstSettingInfo :
 ...
 *
 ******************************************************************************/
void D4Hp3_CheckArgument_Mixer(UINT8 bHpFlag, UINT8 bSpFlag,
D4HP3_SETTING_INFO *pstSettingInfo)
{
...
void D4Hp3_CheckArgument(D4HP3_SETTING_INFO *pstSettingInfo)
{
	...
	if ((pstSettingInfo->bSpNcpl_AttackTime == 0)
		|| (pstSettingInfo->bSpNcpl_AttackTime > 3)) {
 ...
 *			UINT8 bSpFlag : "change SP amp mixer setting" flag
 *(0 : no change,  1 : change)
 *			D4HP3_SETTING_INFO *pstSetMixer :
 *D-4HP3 setting information
 *
 *	Return:
 *			none
 *
 ******************************************************************************/
void D4Hp3_ControlMixer(UINT8 bHpFlag, UINT8 bSpFlag,
D4HP3_SETTING_INFO *pstSetMixer)
{
	...
	if (bSpFlag == 1) {
		...
		bTempSpMixer_Line1 =
		(g_bD4Hp3RegisterMap[7] & 0x20) >> (D4HP3_SP_AMIX & 0xFF);
		bTempSpMixer_Line2 =
		(g_bD4Hp3RegisterMap[7] & 0x10) >> (D4HP3_SP_BMIX & 0xFF);
	}
	...
	if (bHpFlag == 1) {
		...
		bTempHpMixer_Line1 =
		(g_bD4Hp3RegisterMap[7] & 0x02) >> (D4HP3_HP_AMIX & 0xFF);
		bTempHpMixer_Line2 =
		(g_bD4Hp3RegisterMap[7] & 0x01) >> (D4HP3_HP_BMIX & 0xFF);
	}

	/* write register #0x87 */
	bWriteRA = 0x87;
	bWritePrm =
		(bTempSpMixer_Line1 << 5) | (bTempSpMixer_Line2 << 4)
		| (bTempHpCh << 3)
		| (bTempHpMixer_Line1 << 1) | (bTempHpMixer_Line2);
...
void D4Hp3_PowerOn(D4HP3_SETTING_INFO *pstSettingInfo)
{
	...
	/* 0x80 */
	abWritePrm[0] =
		(pstSettingInfo->bHpCpMode << (D4HP3_CPMOD & 0xFF))
		| (pstSettingInfo->bHpAvddLev << (D4HP3_VLEVEL & 0xFF));
	/* 0x81 */
	abWritePrm[1] =
		(pstSettingInfo->bHpEco << (D4HP3_ECO_MODE & 0xFF))
...
void yda165_speaker_onoff(int onoff)
{
	...
	if (onoff) {
		...
		/* input */
		stInfo.bLine1Gain = g_ampgain[cur_mode].in1_gain;
		/* LINE1 Gain Amp */
		stInfo.bLine2Gain = g_ampgain[cur_mode].in2_gain;
		/* LINE2 Gain Amp */

		stInfo.bLine1Balance = 0;
		/* LINE1 Single-ended(0) or Differential(1) */
		stInfo.bLine2Balance = 0;
		/* LINE2 Single-ended(0) or Differential(1) */

		/* HP */
		stInfo.bHpCpMode = 0;
		/* HP charge pump mode setting, 3stage mode(0)
		/ 2stage mode(1) */
		stInfo.bHpAvddLev = 0;
		/* HP charge pump AVDD level, 1.65V<=AVDD<2.40V(0)
		/ 2.40V<=AVDD<=2.86V(1) */
		stInfo.bHpEco = 0;
		/* HP eco mode, normal(0) / eco mode(1) */
		stInfo.bHpAtt = g_ampgain[cur_mode].hp_att;
		/* HP attenuator */
		stInfo.bHpGainUp = g_ampgain[cur_mode].hp_gainup;
		/* HP gain up */
		stInfo.bHpSvol = 0;
		/* HP soft volume setting, on(0) / off(1) */
		stInfo.bHpZcs = 1;
		/* HP zero cross mute setting, on(0) / off(1) */
		stInfo.bHpCh = 0;
		/* HP channel, stereo(0)/mono(1) */
		stInfo.bHpMixer_Line1 = 0;
		/* HP mixer LINE1 setting */
		stInfo.bHpMixer_Line2 = 0;
		/* HP mixer LINE2 setting */

		/* SP */
		stInfo.bSpAtt = g_ampgain[cur_mode].sp_att;
		/* SP attenuator */
		stInfo.bSpGainUp = g_ampgain[cur_mode].sp_gainup;
		/* SP gain up */
		stInfo.bSpSvol = 0;
		/* SP soft volume setting, on(0) / off(1) */
		stInfo.bSpZcs = 0;
		/* SP zero cross mute setting, on(0) / off(1) */
		stInfo.bSpMixer_Line1 = 1;
		/* SP mixer LINE1 setting */
		stInfo.bSpMixer_Line2 = 0;
		/* SP mixer LINE2 setting */
		stInfo.bSpNg_DetectionLv = 0;
		/* SP Noise Gate : detection level */
		stInfo.bSpNg_AttackTime = 1;
		/* SP Noise Gate : attack time */
		stInfo.bSpNcpl_NonClipRatio = 0;
		/* SP Non-Clip power limiter : Non-Clip distortion ratio */
		stInfo.bSpNcpl_PowerLimit = 0;
		/* SP Non-Clip power limiter : Power Limit */
		stInfo.bSpNcpl_AttackTime = 2;
		/* SP Non-Clip power limiter : attack Time */
		stInfo.bSpNcpl_ReleaseTime = 1;
		/* SP Non-Clip power limiter : release Time */
...
void yda165_speaker_onoff_incall(int onoff)
{
	D4HP3_SETTING_INFO stInfo;
#if AMPREG_DEBUG
	unsigned char buf;
	...
	if (onoff) {
		pr_info(MODULE_NAME ":speaker on\n");
		/* input */
		stInfo.bLine1Gain = g_ampgain[1].in1_gain;
		/* LINE1 Gain Amp */
		stInfo.bLine2Gain = g_ampgain[1].in2_gain;
		/* LINE2 Gain Amp */
		...
		/* HP attenuator */
		stInfo.bHpGainUp = g_ampgain[1].hp_gainup;
		/* HP gain up */
		stInfo.bHpSvol = 0;
		/* HP soft volume setting, on(0) / off(1) */
		stInfo.bHpZcs = 1;
		...
		/* SP attenuator */
		stInfo.bSpGainUp = g_ampgain[1].sp_gainup;
		/* SP gain up */
		stInfo.bSpSvol = 0;
		/* SP soft volume setting, on(0) / off(1) */
		stInfo.bSpZcs = 0;
...
void yda165_speaker_onoff_inVoip(int onoff)
{
	...
	if (onoff) {
		...
		/* input */
		stInfo.bLine1Gain = g_ampgain[0].in1_gain;
		/* LINE1 Gain Amp */
		stInfo.bLine2Gain = g_ampgain[0].in2_gain;
		/* LINE2 Gain Amp */
		...
		/* HP attenuator */
		stInfo.bHpGainUp = g_ampgain[0].hp_gainup;
		/* HP gain up */
		stInfo.bHpSvol = 0;
		/* HP soft volume setting, on(0) / off(1) */
		stInfo.bHpZcs = 1;
		...
		/* SP attenuator */
		stInfo.bSpGainUp = g_ampgain[0].sp_gainup;
		/* SP gain up */
		stInfo.bSpSvol = 0;
		/* SP soft volume setting, on(0) / off(1) */
		stInfo.bSpZcs = 0;
...
void yda165_headset_onoff(int onoff)
{
	D4HP3_SETTING_INFO stInfo;
	if (onoff) {
...
void yda165_tty_onoff(int onoff)
{
	D4HP3_SETTING_INFO stInfo;
	if (onoff) {
...
static int yda165_probe(struct i2c_client *client,
