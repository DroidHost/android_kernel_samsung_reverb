...
static void mdp4_lcdc_do_blt(struct msm_fb_data_type *mfd, int enable)
{
	...
	int data;
	struct clk *pixel_lcdc_clk; /* drives the lcdc interface */

	pixel_lcdc_clk = get_pixel_lcdc_clk();
	if (IS_ERR(pixel_lcdc_clk)) {
			pr_info("%s: Couldn't get pixel_lcdc_clk\n", __func__);
			return -EINVAL;
	}
#endif
	do {
	...
	&& defined(CONFIG_FB_MSM_LCDC_R69329_WVGA)
	/*
	* This part use for ICTL panel because of the TSP_VSYNC.
	* The solution is disable -> enalbe the pixel clock also
	* when the time of change to blt mode from normal mode
	* with lcdc timing generator.
	*/
