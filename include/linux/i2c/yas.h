...
#else
	int (*i2c_write)(uint8_t slave, uint8_t addr,
	const uint8_t *buf, int len);
	int (*i2c_read)(uint8_t slave, uint8_t addr,
		uint8_t *buf, int len);
#endif
...
struct yas_acc_driver_callback {
	...
	int (*i2c_open)(void);
	int (*i2c_close)(void);
	int (*i2c_write)(uint8_t slave, uint8_t adr,
		const uint8_t *buf, int len);
	int (*i2c_read) (uint8_t slave, uint8_t adr,
		uint8_t *buf, int len);
	void (*msleep)(int msec);
};
...
struct yas_utility {
	int (*get_rotation_matrix)(struct yas_vector *acc,
		struct yas_vector *mag, struct yas_matrix *matrix);
