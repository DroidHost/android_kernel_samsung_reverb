...
static int brcm_wlan_power(int onoff)
{
	...
	if (onoff) {
		if (gpio_request(GPIO_WLAN_HOST_WAKE, "wlan_host_wake"))
			printk(KERN_INFO "wlan :	%s:gpio_request"\
				"(GPIO_WLAN_HOST_WAKE) failed\n", __func__);

		rc = gpio_request(GPIO_WLAN_REG_ON, "wlan_en");
		if (!rc) {
			rc = gpio_direction_output(GPIO_WLAN_REG_ON,
						 GPIO_WLAN_LEVEL_HIGH);
		}
		if (rc) {
...
{
	struct mmc_host *mmc = platform_get_drvdata(&msm_device_sdc3);

	printk(KERN_INFO"msm_device_sdc3.name= %d\n", msm_device_sdc3.id);
