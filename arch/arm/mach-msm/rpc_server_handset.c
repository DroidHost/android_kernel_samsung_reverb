...
static ssize_t  sysfs_power_key_pressed_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	printk(KERN_INFO "power_key is %s\n",
				power_key_pressed ? "pressed" : "released");
	return snprintf(buf, 5, "%d\n", power_key_pressed);
}

static DEVICE_ATTR(sec_power_key_pressed, 0664 , sysfs_power_key_pressed_show,
	NULL);
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	switch (key) {
	case KEY_POWER:
			power_key_pressed = (key_code != HS_REL_K);
	case KEY_END:
	case KEY_MEDIA:
	case KEY_VOLUMEUP:
	case KEY_VOLUMEDOWN:
...
static int __devinit hs_probe(struct platform_device *pdev)
{
	...
	sec_power_key = device_create(sec_class, NULL, 0, NULL,
			"sec_power_key");

	if (IS_ERR(sec_power_key))
		pr_err("Failed to create device(sec_power_key)!\n");

	rc = device_create_file(sec_power_key, &dev_attr_sec_power_key_pressed);
	if (rc) {
		pr_err("Failed to create device file in sysfs entries(%s) err(%d)!\n",
				dev_attr_sec_power_key_pressed.attr.name, rc);
	}

	dev_set_drvdata(sec_power_key, hs);
